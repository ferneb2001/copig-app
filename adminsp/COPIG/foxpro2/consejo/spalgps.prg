procedure spalgps
* 01/04/2003
*graba pagos para san martin
*  provisorio con alternativas
* incluye pagos realizados
...
if found()
	if b.dpto=8 or b.dpto=9 or b.dpto=10 or b.dpto=11
		wop=0
		select 5
		seek(a.titulo)
		if found()
    ...
    if wop=0  
	  ...
	  do while found()
	  	select 3
	  	append blank
	  	replace anhabil with h.anhabil
	  	replace fecha   with h.fecha
	  	replace matric  with h.matric
	  	replace cpto    with h.cpto
	  	replace catego  with h.catego
	  	replace importe with h.importe
	  	replace recibo  with h.recibo
	  	replace detalle with h.detalle
	  	select 8
	  	continue
	  enddo
	endif
