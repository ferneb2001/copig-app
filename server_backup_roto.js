...
// [CORREGIDO] Import duplicado removido
const fs = require('fs');
...
app.get('/api/chp/documentos/:solicitudId/:categoria?', async (req, res) => {
    try {
        ...
        // Validar que solicitudId sea un número
        if (isNaN(parseInt(solicitudId))) {
            return res.json({
                success: true,
                documentos: [],
                message: `Parámetro solicitudId inválido: ${solicitudId}`
            });
        }
        ...
        `;
        const params = [parseInt(solicitudId)];
