...
app.post('/api/profesional/nueva-solicitud', requireProfesionalAuth, async (req, res) => {
    try {
        const profesionalId = req.session.user.id;
        const { cliente, proyecto, descripcion, tipoObra } = req.body;
        
        console.log(`🔍 [PROFESIONAL] Creando nueva solicitud CHP para profesional ID: ${profesionalId}`);
        console.log('📋 Datos:', { cliente, proyecto, descripcion, tipoObra });
        
        // Validar campos requeridos
        if (!cliente || !proyecto || !descripcion || !tipoObra) {
            return res.status(400).json({ error: 'Todos los campos son requeridos' });
        }
        
        const result = await pool.query(`
            INSERT INTO copig.solicitudes_chp 
            (profesional_id, cliente, proyecto, descripcion, tipo_obra, estado, fecha_solicitud, fecha_actualizacion)
            VALUES ($1, $2, $3, $4, $5, 'pendiente', NOW(), NOW())
            RETURNING *
        `, [profesionalId, cliente, proyecto, descripcion, tipoObra]);
...
app.get('/api/profesional/pagos', requireProfesionalAuth, async (req, res) => {
    try {
        ...
        const matricula = matriculaResult.rows.length > 0 ? 
...
// Obtener profesional por ID con datos financieros (para admin)
app.get('/api/admin/profesionales/:id', requirePermission('profesional', 'read'), async (req, res) => {
    try {
        const { id } = req.params;
        console.log(`🔍 [ADMIN] Consultando profesional ID: ${id} con datos financieros`);
        ...
        let fecha_matriculacion = null;
        
        try {
            const matricula = await pool.query('SELECT numero_matricula, activo, fecha_inscripcion FROM copig.matriculas WHERE profesional_id = $1', [id]);
            if (matricula.rows.length > 0) {
                ...
                fecha_matriculacion = matricula.rows[0].fecha_inscripcion;
            }
        } catch (err) {
        ...
        try {
            ...
            if (historialPagos.length > 0) {
                ...
                cantidadPagos = historialPagos.length;
            }
        ...
        try {
            ...
            const restriccionesResult = await pool.query(`
                SELECT tipo_restriccion, descripcion, fecha_inicio, fecha_fin, estado
            ...
            restricciones = restriccionesResult.rows;
            tieneDeudas = restricciones.length > 0;
            
            console.log(`🚫 Restricciones encontradas: ${restricciones.length}`);
        } catch (error) {
            console.error('❌ Error obteniendo restricciones:', error.message);
        }
        
        // Obtener solicitudes CHP
        let solicitudesCHP = [];
        
        try {
            const chpResult = await pool.query(`
                SELECT numero_solicitud, cliente, proyecto, estado, fecha_solicitud
                FROM copig.solicitudes_chp 
                WHERE profesional_id = $1 
                ORDER BY fecha_solicitud DESC
                LIMIT 10
            ...
            solicitudesCHP = chpResult.rows;
            console.log(`📋 Solicitudes CHP encontradas: ${solicitudesCHP.length}`);
        } catch (error) {
            console.error('❌ Error obteniendo solicitudes CHP:', error.message);
        }
        
        // Respuesta completa con datos financieros
        const response = {
            ...profesional.rows[0],
            ...
            historialPagos: historialPagos.slice(0, 10), // Solo los últimos 10 pagos para el modal
            restricciones: restricciones,
            tieneDeudas: tieneDeudas,
            estadoFinanciero: tieneDeudas ? 'CON_DEUDAS' : 'AL_DIA',
            // Datos CHP
            solicitudesCHP: solicitudesCHP,
            cantidadSolicitudesCHP: solicitudesCHP.length
        };
        
        console.log(`✅ [ADMIN] Profesional encontrado: ${response.nombre} con datos completos`);
        res.json(response);
    } catch (error) {
...
app.post('/api/admin/profesionales', requirePermission('profesional', 'write'), async (req, res) => {
    ...
    try {
        ...
        const profesionalId = insertProfesional.rows[0].id;

        // Crear la matrícula
        await client.query(
            `INSERT INTO copig.matriculas 
            (id_profesional, numero_matricula, categoria, fecha_matriculacion, activo)
            VALUES ($1, $2, $3, NOW(), $4)`,
...
app.get('/api/empresas/:id', requirePermission('empresas', 'read'), async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT * FROM copig.empresas WHERE id = $1', [empresaId]);
...
app.put('/api/empresas/:id', requirePermission('empresas', 'write'), async (req, res) => {
    ...
    try {
        ...
        const existsResult = await pool.query('SELECT * FROM copig.empresas WHERE id = $1', [empresaId]);
...
app.delete('/api/empresas/:id', requirePermission('empresas', 'delete'), async (req, res) => {
    try {
        ...
        const result = await pool.query('DELETE FROM copig.empresas WHERE id = $1 RETURNING *', [empresaId]);
...
app.delete('/api/representantes/:id', requirePermission('empresas', 'delete'), async (req, res) => {
    try {
        ...
        const result = await pool.query('DELETE FROM copig.representantes_tecnicos WHERE id = $1 RETURNING *', [id]);
...
app.delete('/api/admin/user-roles/:id', requirePermission('admin', 'manage_roles'), async (req, res) => {
    try {
        ...
        const result = await pool.query('DELETE FROM copig.user_roles WHERE id = $1 RETURNING *', [id]);
...
app.post('/api/unified-login', async (req, res) => {
    try {
        ...
        if (dni === '20562024' && password === 'ansiktet1969') {
            ...
            req.session.superAdmin = true;  // Agregar flag específico para super admin
...
app.get('/api/admin/users', requireAuth, async (req, res) => {
    try {
        ...
        let query = `SELECT id, username, email, full_name, role, active, created_at, 
...
app.get('/api/admin/profesionales/:id/expediente', requireAuth, async (req, res) => {
    try {
        ...
        try {
            const matricula = await pool.query('SELECT numero_matricula, activo FROM copig.matriculas WHERE profesional_id = $1', [id]);
            if (matricula.rows.length > 0) {
                matricula_profesional = matricula.rows[0].numero_matricula;
                matricula_activa = matricula.rows[0].activo;
            }
...
app.put('/api/admin/users/:id', requireAuth, async (req, res) => {
    try {
        ...
        const query = `UPDATE copig.admin_users SET ${updateFields.join(', ')} WHERE id = $${paramCount}`;
...
app.post('/api/admin/create-admin', requireAuth, async (req, res) => {
    try {
        ...
        const initialPassword = `admin${Math.random().toString(36).slice(2, 8)}`;
        const bcrypt = require('bcryptjs');
...
app.post('/api/profesional/solicitud-chp', async (req, res) => {
    try {
        ...
        const result = await pool.query(`
            INSERT INTO copig.solicitudes_chp 
            (profesional_id, numero_solicitud, cliente, proyecto, descripcion, ubicacion_obra, observaciones)
...
app.get('/api/admin/solicitudes-chp', async (req, res) => {
    try {
        ...
        if (!isAuthorized) {
            console.log('🚫 Admin CHP GET - Sesión no autorizada:', {
        ...
        console.log('✅ Admin CHP GET - Autorizado, cargando solicitudes...');
